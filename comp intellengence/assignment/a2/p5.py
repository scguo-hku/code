...
def min_max_multiple_ghosts(problem, k):
    ...
    def minimax(state, depth, agent_index, max_depth, alpha=float('-inf'), beta=float('inf')):
        ...
        if agent_index == 0:
            ...
            return getMax(state, depth, max_depth, agent_index, alpha, beta)
        else:
            # Ghost's turn: minimizing player
            return getMin(state, depth, max_depth, agent_index, alpha, beta)

    def getMax(state, depth, max_depth, agent_index, alpha, beta):
        pacman_pos, ghost_positions, layout = state
        ...
        for move in directions:
            ...
            value, _ = minimax(new_state, depth + 1, agent_index + 1, max_depth, alpha, beta)
    ...
    def getMin(state, depth, max_depth, agent_index, alpha, beta):
        pacman_pos, ghost_positions, layout = state
        best_value = float('inf')
        best_move = None
        ...
        for move in directions:
            ...
            value, _ = minimax(new_state, depth + 1, next_agent_index, max_depth, alpha, beta)
    ...
    while not game_over:
        ...
        if not game_over:
            ...
            for ghost_index, ghost_pos in enumerate(ghost_positions):
                ...
                # Update layout
                if moves:
                    # Use Minimax to choose the best direction to move the Ghost
                    _, ghost_move = minimax((pacman_pos, ghost_positions, layout), 0, ghost_index + 1, max_depth)
                    new_ghost_pos = {
                        'N': (i - 1, j),
                        'S': (i + 1, j),
                        'W': (i, j - 1),
